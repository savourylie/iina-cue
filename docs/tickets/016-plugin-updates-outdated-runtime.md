...
### 2026-09-26 (IINA test and changes from it)
- In IINA on this Mac, the user pressed Update and runtime 0.1.6 became 0.1.9. The stale 0.1.6 archive was removed first. The download took 16 s from Hugging Face and passed its checksum. The old helper had lost this window's lease, answered `CLIENT_REQUIRED` to its release, and still accepted the shutdown. The unpack swapped `runtime/` at 11:47:09. The archive and `runtime.previous` were removed, and the new helper passed the test clip in 8.3 s. The `models` symlink and all 99 cached caption chunks were unchanged. AI subtitles worked afterwards (user).
- The user's findings, and what changed:
  1. The offer card filled the top of the sidebar, with no space before the controls, and could not be ignored. An offer is now only a small "Update Cue" button at the top right, with the size and the caption pause in its tooltip. A running or failed update shows a short status block with a separator and 16 px before the controls. The controls stay visible, because each window turns its own captions off and on.
  2. There was no progress during the update. Root cause: IINA 1.4.4 resolves every `utils.exec` from one serial queue per plugin instance (`pluginInstance.queue`, `DispatchQueue(label:qos:)`). So the `stat` size checks, and the `sysctl`/`sw_vers`/`df` facts that every setup redraw read, waited behind curl and then behind the unpack. The download size is now read through `iina.file.handle` (`seekToEnd`, `offset`), which does not use that queue. A setup run reads the Mac's facts once and reuses them. Unpacking, the test clip, and a download before its first byte count show an indeterminate bar. First-run setup had the same flaw, and this fixes it too.
  3. The E4B credit took several lines. It is now a "Credits" disclosure, collapsed by default.
  4. The Remove button sat under the model's text as a native bezel button. Download, Resume, Cancel and Remove now end the model's name row, flat and in the accent color like the other Advanced actions.
  6. The user had to turn captions off by hand in every window. The update now keeps captions running during the download. Right before the swap, every window turns its captions off: this one directly, others through a relay in the plugin's global instance (`global.ts`). IINA's global broadcast only reaches plugin-created windows, so each window says hello and is addressed by its label. After the test clip, captions come back without pausing playback (`start({hold: false})`). A window where the user switches captions on during the swap starts them after it. Only an MKV copy still refuses the update; that is the user's work. `stopHelper` retries for about 6 s while other windows release their leases.
- Verification: 91 plugin tests. The end-to-end test on the real bundle now starts with captions on and checks this order: start 0.1.6, captions on helper-old, download, verify, captions off, release lease, shutdown, swap runtime, start 0.1.9, test clip on helper-new, captions on helper-new. The relay has its own test. Checked in Chrome at 240 and 320 px: the button, the status block with a determinate and an indeterminate bar, and the model rows with collapsed credits, with no overflow. `scripts/test` (203 helper tests) and `npm run build` pass. The new flow has not run in IINA yet: this Mac already has 0.1.9, so it needs an older runtime put back to show the offer.
