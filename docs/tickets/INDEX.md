...
| 014 | [Publish the plugin release](./014-publish-plugin-release.md) | `pending` | #013 ✅ | Confirm how IINA's Install from GitHub finds a plugin first; user approves the publish |
...
| 2 | #013: general-user plugin build | ✅ done | `npm run build:release` makes `dist/Cue-<version>.iinaplgz` without this checkout's paths |
| 3 | #014: published plugin release | 📋 pending, ready to start | Both install routes in 012 need a general-user `.iinaplgz` on a GitHub Release. Publishing needs the user's approval at the time |
