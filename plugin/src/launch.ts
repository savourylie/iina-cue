...
export function planLaunch(input: LaunchInput): LaunchPlan {
  ...
  if (preference) {
    ...
    return {file: preference, args: ["ensure"]};
  }
  ...
  if (input.exists(marker)) {
    ...
    if (input.exists(launcher)) {
      ...
      if (absolute && absolute.startsWith("/")) return {file: "/bin/sh", args: [absolute, "ensure"]};
      return {file: launcher, args: ["ensure"]};
    }
    if (input.exists(python)) {
      const absolute = input.resolve(python);
      if (absolute && absolute.startsWith("/")) return {file: absolute, args: ["-m", "cue.cli", "ensure"]};
      return {file: python, args: ["-m", "cue.cli", "ensure"]};
    }
    throw new Error("SETUP_REQUIRED");
  }
  if (!input.devBootstrap.startsWith("/") || !input.exists(input.devBootstrap)) throw new Error("SETUP_REQUIRED");
  return {file: input.devBootstrap, args: ["ensure"]};
}
