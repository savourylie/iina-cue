...
- `models/` — license text for the pinned Gemma 4 E2B, E4B and 12B and Qwen3 ForcedAligner artifacts, and for the Silero VAD model. The Gemma and Qwen weights are not inside the runtime; downloading them remains a separate consent step. E4B and 12B are downloaded only when the user chooses one in Advanced. The Silero VAD model is inside the runtime as `vad/silero_vad.onnx`.
...
Two larger Gemma 4 models are optional downloads, pinned in `models/manifest.json` under `optional_assets`: Gemma 4 E4B is `litert-community/gemma-4-E4B-it-litert-lm` at revision `2eee7ac325f20eb8c9ac1d0e972f7c84663062da`, and Gemma 4 12B is `litert-community/gemma-4-12B-it-litert-lm` at revision `7a0b1ce0ea821bcd01c5f72af84155e02191152f`. Both model cards at those revisions declare `license: apache-2.0`. Neither repository is gated, and neither revision contains a LICENSE file. `licenses/models/gemma-4-e4b/LICENSE` and `licenses/models/gemma-4-12b/LICENSE` are the Apache License, Version 2.0.
