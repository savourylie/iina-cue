...
A larger Gemma 4 model is an optional download, pinned in `models/manifest.json` under `optional_assets`: Gemma 4 E4B is `litert-community/gemma-4-E4B-it-litert-lm` at revision `2eee7ac325f20eb8c9ac1d0e972f7c84663062da`. The model card at that revision declares `license: apache-2.0`. The repository is not gated, and the revision does not contain a LICENSE file. `licenses/models/gemma-4-e4b/LICENSE` is the Apache License, Version 2.0.
